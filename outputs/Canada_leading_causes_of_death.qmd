...
To explore and visualize the data, we used a suite of R packages, each chosen for its specific capabilities. The dplyr package facilitated efficient data manipulation, while ggplot2 helped us visualize trends and patterns in mortality. We used Poisson and negative binomial regression techniques to model the data, and rstanarm was chosen for its advanced Bayesian modeling capabilities. The modelsummary package efficiently summarized the results, providing clear and concise insights into the findings. We also utilized the broom and broom.mixed packages to tidy the outputs of our statistical models, making the results more accessible and interpretable.
